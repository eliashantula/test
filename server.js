...
Reply.findByIdAndUpdate(req.body.id, {$push: {replies: reply._id }}, {new: true}).then((response)=>{
	
Reply.find({}).populate({path: 'replies'}).exec((err,posts)=>{
console.log(posts)
...
Reply.remove({}).exec().then((response)=>{
...
app.post('/post', (req, res ,next)=>{
	...
	reply.save().then((reply)=>{
	Reply.find({}).populate('replies').then((posts)=>{
